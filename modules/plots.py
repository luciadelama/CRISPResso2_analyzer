...
def _grouped_barplot(df, value_col, title, ylabel, treat_order=None):
    # Requeridos: columnas "Treatment", "Replicate", y la métrica (value_col)
    required = {"Treatment", "Replicate", value_col}
    if df.empty or not required.issubset(df.columns):
        return None

    # Agregar por Treatment x Replicate (por si hay varias filas por combinación)
    agg = (
        df.groupby(["Treatment", "Replicate"], as_index=False)[value_col]
          .mean()  # o .sum() según tu lógica
    )
    ...
    # Plot: una barra por réplica, desplazada dentro del grupo
    for i, rep in enumerate(replicates):
        ...
        ax.bar(x + offset, y, width, label=str(rep))
...
def make_frameshift_plot(df, treat_order=None):
    return _grouped_barplot(df, value_col="Frameshift%", title="Frame shift % per Treatment", ylabel="Frame shift %", treat_order=treat_order)
